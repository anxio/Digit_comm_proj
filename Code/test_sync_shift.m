...
nr_errors_ps = zeros(1, length(EbN0_db));   % Error counter
...
for snr_point = 1:length(EbN0_db)
  ...
  for blk = 1:nr_blocks
    ...
    temp_ps=bhat_ps(1+nr_training_bits:nr_training_bits+nr_data_bits) ~= b_data;
    nr_errors_ps(snr_point) = nr_errors_ps(snr_point) + sum(temp_ps);
...
BER_ps = nr_errors_ps / nr_data_bits / nr_blocks;
...
%plot(EbN0_db,(nr_guard_bits/nr_bits_per_symbol+1)*Q*ones(1,length(EbN0_db)),'.-');
plot(EbN0_db,((timeshift-1)+(nr_guard_bits/nr_bits_per_symbol+1)*Q)*ones(1,length(EbN0_db)),'.');
l=legend('Estimate','timeshift+(nr_{guard_bits}/nr_{bits_per_symbol}+1)*Q}');
set(l,'interpreter','none')
set(gca, 'FontSize', 13);
ylabel('Average on time-shift correction')
xlabel('E_b / N_0 in dB');
% ylim([ ((nr_guard_bits/nr_bits_per_symbol+1)*Q-2) (timeshift-1+(nr_guard_bits/nr_bits_per_symbol+1)*Q+10)])
title(strcat('Study of timing synchronization in function of E_b/N_0 with \tau =',32,num2str(timeshift-1)));

subplot(212)
hold all
plot(EbN0_db,BER);
plot(EbN0_db,BER_ps);
legend('without phase correction','with phase correction')
set(gca, 'FontSize', 13);
ylabel('BER')
xlabel('E_b / N_0 in dB');
