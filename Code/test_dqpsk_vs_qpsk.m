...
nr_errors_dqpsk = zeros(1, length(EbN0_db));
...
for snr_point = 1:length(EbN0_db)
  ...
  for blk = 1:nr_blocks
    ...
    tx_dqpsk = upfirdn(d_dqpsk, pulse_shape, Q, 1);
    
    % phase shift
    theta=(1:length(tx))/length(tx)*pi/4;
    tx=tx.*exp(1i*theta);
    tx_dqpsk=tx_dqpsk.*exp(1i*theta);
    ...
    mf_dqpsk=conv(mf_pulse_shape,rx_dqpsk);
...
%     t_samp_dqpsk = sync(mf_dqpsk, b_train, Q, t_start, t_end);
%     t_samp = sync(mf, b_train, Q, t_start, t_end);
    
    t_samp_dqpsk = (nr_guard_bits/nr_bits_per_symbol+1)*Q;
    t_samp = (nr_guard_bits/nr_bits_per_symbol+1)*Q;
    ...
    r_dqpsk=mf_dqpsk(t_samp_dqpsk:Q:t_samp_dqpsk+Q*(nr_training_bits+nr_data_bits)/2-1);
    ...
    temp_dqpsk=bhat_dqpsk(1+nr_training_bits:nr_training_bits+nr_data_bits) ~= b_data;
    nr_errors_dqpsk(snr_point) = nr_errors_dqpsk(snr_point) + sum(temp_dqpsk);
...
BER_dqpsk = nr_errors_dqpsk / nr_data_bits / nr_blocks;
...
plot(EbN0_db,BER);
plot(EbN0_db,BER_dqpsk);
legend('Ideal BER','BER with QPSK ','BER with DQPSK')
xlabel('E_b / N_0 in dB');
ylabel('Bit error rate');
title('Studying of the performance of the DQPSK againt QPSK for phase shift theta=(t/T)*\pi/4')
